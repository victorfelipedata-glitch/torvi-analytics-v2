...
st.markdown("""
    ...
    /* Tarjetas de Picks (Reemplazo de Expanders) */
...
base_bankroll = float(user_doc.to_dict().get('bankroll', 1000.0)) if user_doc.exists else 1000.0

# Obtener los picks que este usuario ha agregado a su portafolio
...
def calcular_kelly(ev_pct, cuota, bankroll):
    # Fórmula Half-Kelly
    if cuota <= 1: return 0
    p = (ev_pct / 100) + (1 / cuota) # Aproximación de prob real basada en EV
    b = cuota - 1
    kelly_frac = (p * b - (1 - p)) / b
    half_kelly = max(0, kelly_frac * 0.5)
    inversion = bankroll * half_kelly
    return inversion, half_kelly * 100
...
if not df.empty:
    ...
    # --- PESTAÑAS (Dependiendo del rol mostramos Admin) ---
    ...
    with paneles[0]:
        if not df_normales.empty:
            for i, r in df_normales.iterrows():
                pid = r['id']
                inv_recomendada, pct_bank = calcular_kelly(r['ev'], r['cuota'], base_bankroll)
                
                # Diseño de Tarjeta Limpia
                st.markdown(f"""
                <div class="pick-card">
                    <div class="pick-header">
                        <h3 class="pick-title">{str(r['partido']).replace('[SENCILLA]','')}</h3>
                        <span class="pick-ev">EV+ {r['ev']}%</span>
                    </div>
                    <p style="color: #E2E8F0; font-size: 1.1rem; margin-bottom: 15px;">🎯 {r['mercado']}</p>
                    <div class="pick-stats">
                        <div class="stat-box"><span>Cuota</span><div class="stat-val">{r['cuota']}</div></div>
                    ...
                    <div class="kelly-box">
                        💡 <b>Criterio Kelly:</b> Se recomienda invertir el <b>{pct_bank:.1f}%</b> de tu Bankroll (${inv_recomendada:.2f})
                    </div>
                    <p style="color: #94A3B8; font-size: 0.9rem; line-height: 1.5;">{r.get('analisis','')}</p>
                </div>
                """, unsafe_allow_html=True)
                
                # Botones funcionales debajo de la tarjeta
                c_btn, c_space = st.columns([1, 3])
                with c_btn:
                    if pid in mis_picks_dict:
                        st.success("✅ En tu Portafolio")
                    else:
                        if st.button("📥 Añadir a mi Portafolio", key=f"add_{pid}"):
                            db.collection('usuarios').document(user_email).collection('portafolio').document(pid).set({
                                'pick_id': pid, 'stake': inv_recomendada, 'fecha': datetime.now()
                            })
                            st.rerun()
                st.markdown("<br>", unsafe_allow_html=True)
        else:
        ...
        if not df_parlays.empty:
            for i, r in df_parlays.iterrows():
                ...
                <div class="pick-card vip">
                    <div class="pick-header">
                        <h3 class="pick-title" style="color:#F59E0B;">👑 {str(r['partido']).replace('[PARLAY]','').strip()}</h3>
                        <span class="pick-ev" style="background:rgba(245,158,11,0.1); color:#F59E0B;">EV+ {r['ev']}%</span>
                    </div>
                    <p style="color: #E2E8F0; font-size: 1.1rem; margin-bottom: 15px;">🎯 {r['mercado']}</p>
                    <div class="pick-stats">
                        <div class="stat-box" style="border-color:#453300;"><span>Cuota Combinada</span><div class="stat-val" style="color:#F59E0B;">{r['cuota']}</div></div>
                    </div>
                    <div class="kelly-box" style="background:rgba(245,158,11,0.05); border-color:#F59E0B; color:#FCD34D;">
                        💡 <b>Gestión de Riesgo:</b> Inversión recomendada de ${inv_recomendada:.2f}
                    </div>
                    <p style="color: #94A3B8; font-size: 0.9rem; line-height: 1.5;">{r.get('analisis','')}</p>
                </div>
                """, unsafe_allow_html=True)
                
                if pid not in mis_picks_dict:
                    if st.button("📥 Seguir Parlay", key=f"add_p_{pid}"):
                        db.collection('usuarios').document(user_email).collection('portafolio').document(pid).set({
                            'pick_id': pid, 'stake': inv_recomendada, 'fecha': datetime.now()
                        })
                        st.rerun()
                else: st.success("✅ Parlay guardado en tu portafolio.")
                st.markdown("<br>", unsafe_allow_html=True)
        else:
    ...
    with paneles[2]:
        ...
        # Cruzar datos del portafolio con el DB global
        mis_picks_completos = []
            ...
            if not activos_mios.empty:
                for i, r in activos_mios.iterrows():
                    ...
                    <div style="background:#0F172A; padding:15px; border-radius:8px; border-left:3px solid #3B82F6; margin-bottom:10px; display:flex; justify-content:space-between; align-items:center;">
                        <div>
                            <span style="color:#E2E8F0; font-weight:bold;">{r['partido']}</span><br>
                            <span style="color:#64748B; font-size:0.8rem;">{r['mercado']} | Cuota {r['cuota']}</span>
                        </div>
                        <div style="text-align:right;">
                            <span style="color:#94A3B8; font-size:0.8rem;">Invertido</span><br>
                            <span style="color:#10B981; font-weight:bold; font-family:JetBrains Mono;">${r['mi_stake']:.2f}</span>
                        </div>
                    </div>
                    """, unsafe_allow_html=True)
            ...
            if not resueltos_mios.empty:
                ...
                for i, r in resueltos_mios.iterrows():
                    ...
                    <div style="background:#0F172A; padding:10px 15px; border-radius:8px; border-left:3px solid {color}; margin-bottom:5px; opacity: 0.8;">
                        <span style="color:#E2E8F0; font-size:0.9rem;">{icon} {r['partido']} ({r['mercado']})</span>
                    </div>
                    """, unsafe_allow_html=True)
    ...
    with paneles[3]:
        ...
        st.write("Axiom Data utiliza la fórmula de Half-Kelly para calcular tu riesgo basado en tu Bankroll configurado.")
    ...
    # === PESTAÑA 5: ADMIN (Solo visible para ti) ===
        ...
        with paneles[4]:
            ...
            if not df_activos.empty:
                for i, r in df_activos.iterrows():
                    ...
                    ca.write(f"{r['partido']} - {r['mercado']}")
